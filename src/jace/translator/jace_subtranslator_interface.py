...
class JaCeSubTranslatorInterface:
    ...
    Subtranslators are rather simple objects that only have to perform the translation.
    The translation process itself is managed by a driver object, which owns and manage the subtranslators.
    In the end this implements the delegation pattern.

    A subtranslator uses its `get_handled_primitives()` function to indicate for which Jax primitives it want to register.
    It is important that a subtranslator can register for as many primitive it wants.
    At the same time, it is possible that multiple subtranslators have registered for a single primitive.

    If multiple subtranslator have registered for the same primitive they will be ordered by driver.
    There are two ways how a subtranslator can influence this order.
    The first one is by implementing `get_priority()`, the driver will then put them in ascending order.
    I.e. the lower its priority the earlier a subtranslator is checked.
    Subtranslators that returns the special value `JaCeSubTranslatorInterface.DEFAULT_PRIORITY` are handled specially.
    Such subtranslators are _always_ put at the end of the list (in unspecific order).
    ...
    If a subtranslator overrides this functions then it should also override `get_priority()` to return `NotImplemented`.
    ...
    def get_handled_primitives(self) -> Collection[str] | str:
        """Returns the names of all Jax primitives that this subtranslator can handle.

        There is no limit on the number of primitives for which a subtranslator can register.
    ...
    def translate_jaxeqn(
        ...
        Before the driver calls this function it will perform the following preparatory tasks:
        ...
        - It will collect the names of the SDFG variables that are used as input and place them in `in_var_names`, in the same order as `eqn.invars`.
            ...
            It is not allowed to modify the data descriptors these variables refers to in any way, including replacing them.
        - The driver will create a new terminal state and pass it as `eqn_state` argument.
            ...
            A subtranslator should construct the data flow graph inside it.

        It is allowed that the subtranslators creates more states if needed, but this state machine has to have a single terminal state.
        However, state have to be reachable from the passed `eqn_state`.
        In this case the function must return this state explicitly.
        If the function returns `None` the driver will assume that subtranslator was able to fully construct the dataflow graph within `eqn_state`.

        While a subtranslator is forbidden from meddling with the input variables, it is allowed to modify the output variables.
        For example he could create a new SDFG variable, with different strides.
        But if the SDFG variable has a different name the subtranslator has to update the name mapping of the driver, and write this change into `out_var_names`.
    ...
    def get_priority(self) -> int:
        ...
        The value returned by this function is used by the driver to order the subtranslators that have registered for the same primitive.
        ...
        Notes:
            ...
            If a subtranslator instead overrides `__lt__()` this function should return `NotImplemented`.
        """
    ...
    def __lt__(
        ...
        As outlined in the class description this is the second possibility to influence the order of the subtranslator.
        This function should return `True` if `self` should be checked for applicability _before_ `other`.
    ...
    def __eq__(
        ...
        However, if the behaviour of a subtranslator strongly depend on its configuration this function should be overridden.

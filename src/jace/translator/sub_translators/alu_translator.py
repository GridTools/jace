...
from typing import Union, Any, Final, TYPE_CHECKING, cast
...
class ALUTranslator(jtranslator.JaCeSubTranslatorInterface):
    ...
    self.m_unarryOps: Final[dict[str, str]] = {
            "pos":          "__out0 = +(__in0)",
            "neg":          "__out0 = -(__in0)",
    ...
    self._binarryOps: Final[dict[str, str]] = {
    ...
    def translate_jaxeqn(
        ...
        if(not all(invar.aval.shape == ()  for invar, x in zip(eqn.invars, in_var_names) if x is None)):
            raise NotImplementedError(f"Can not handle Literals that are not scalars.")
        if(in_var_names[0] is None):
            raise NotImplementedError(f"Literal can only not be on the right hand side of the operation.")
        if(not any(invar.aval.shape == eqn.outvars[0].aval.shape  for invar in eqn.invars)):
            raise NotImplementedError(f"At least input must have the same shape as the output.")
        ...
        inps_same_shape   = all([eqn.invars[0].aval.shape == eqn.invars[i].aval.shape  for i in range(1, len(eqn.invars))])
        ...
        dims_to_bcastl: Sequence[int] = []
        dims_to_bcastr: Sequence[int] = []
        ...
        if(is_scalar):
            ...
            raise NotImplementedError(f"Only literals an input is only allowed for the scalar case.")
            ...
            assert (not has_some_literals) or all(invar.aval.shape == eqn.outvars[0].aval.shape  for (invar, x) in zip(eqn.invars, in_var_names) if x is not None)
            assert (not has_scalars_as_inputs) or all(invar.aval.shape in {eqn.outvars[0].aval.shape, ()}  for (invar, x) in zip(eqn.invars, in_var_names) if x is not None)
            ...
            inp_shpr = tuple(eqn.invars[1].aval.shape)   # Shape of the right/second input; this must be "expanded"
        ...
        tskl_map_ranges: list[tuple[str, str]] = [
                (f'__i{dim}', f'0:{N}')
                for dim, N in enumerate(eqn.outvars[0].aval.shape)
        ]
        tskl_outputs: list[Union[tuple[str, dace.Memlet]], tuple[None, None]] = []
        tskl_inputs: list[tuple[str, dace.Memlet]] = []

        # Generate the Memlets for the input.
        for i, dims_to_bcast in zip(range(len(eqn.invars)), [dims_to_bcastl, dims_to_bcastr]):
            if(in_var_names[i] is None):
                # Litteral: No input needed.
                tskl_inputs.append((None, None))
                continue
            elif(inp_scalars[i]):
                # We have a scalar argument.
                i_memlet = dace.Memlet.from_array(in_var_names[i], translator.getSDFG().arrays[in_var_names[i]])
            else:
                ...
                i_memlet: dace.Memlet = dace.Memlet.simple(in_var_names[i], ", ".join(tInputs_))
                del inputs_
            tskl_inputs.append( (f'__in{i}', i_memlet) )

        # Now generate the Memlets for the outputs
        if(is_scalar):
            tskl_outputs.append( (f'__out{i}', dace.Memlet.from_array(out_var_names[0], translator.getSDFG().arrays[out_var_names[i]])) )
        else:
            tskl_outputs.append( (f'__out{i}', dace.Memlet.simple(out_var_names[0], ', '.join([X[0]  for X in tskl_map_ranges]))) )

        if(is_scalar):
            tskl_tasklet = eqn_state.add_tasklet(
                    tskl_name,
                    list_to_dict(tskl_inputs).keys(),
                    list_to_dict(tskl_outputs).keys(),
                    tskl_code,
            )
            for in_var, (in_connector, in_memlet)  in filter(lambda X: X[0] is not None, zip(in_var_names, tskl_inputs)):
                eqn_state.add_edge(
            ...
            for out_var, (out_connector, out_memlet) in zip(out_var_names, tskl_outputs):
                out = eqn_state.add_write(oVar)
                eqn_state.add_edge(
                        tskl_tasklet,
                        out_connector,
                        eqn_state.add_write(oVar),
                        None,
                        out_memlet,
                )
        else:
    ...
    def _writeTaskletCode(
            self,
            in_var_names: list[Union[str, None]],
            eqn: JaxprEqn,
    ):
        ...
        if("integer_pow" == t_name):
            # INTEGER POWER
            exponent = int(eqn.params['y'])
            if(exponent == 0):
                t_code = f"__out0 = dace.{str(eqn.outvars[0].aval.dtype)}(1)"
            elif(exponent == 1):
                t_code = "__out0 = __in0"
            elif(exponent == 2):
                ...
                t_code = self.m_unarryOps[t_name]
        ...
        for i, in_var_name in enumerate(in_var_names):
            ...
            if(jax_in_var.aval.shape == ()):
                ...
                raise ValueError(f"Can not handle the literal case of shape: {jax_in_var.aval.shape}")
